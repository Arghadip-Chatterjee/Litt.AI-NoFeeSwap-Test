...
const unlockInterface = new ethers.Interface(UNLOCK_ABI);

const NOFEESWAP_ADDR = ADDRESSES.nofeeswap?.toLowerCase();
const OPERATOR_ADDR  = ADDRESSES.operator?.toLowerCase();
...
export interface DecodedSwap {
  ...
  poolId:          bigint;
  amountSpecified: bigint;   // signed, negative = exact input
  limitOffsetted:  bigint;   // the offsetted uint64 limit from VM bytecode
  logPriceLimit:   bigint;   // un-offsetted limit (limitOffsetted - 2^63)
  zeroForOne:      bigint;   // 0 = oneForZero, 1 = zeroForOne, 2 = auto-detect
 ...
 */
function parseVmBytecode(data: Uint8Array): {
  ...
  while (ptr < end) {
    ...
    if (opcode === SWAP_OP) {
      ...
      let pid = 0n;
      for (let i = 0; i < 32; i++) pid = (pid << 8n) | BigInt(data[p + i]);
      p += 32;
      poolId = pid;
      ...
      let lim = 0n;
      for (let i = 0; i < 8; i++) lim = (lim << 8n) | BigInt(data[p + i]);
      p += 8;
      limitOffsetted = lim;
  ...
  if (!swapFound || amountSpecified === null) return null;

  return { amountSpecified, poolId, limitOffsetted, zeroForOne };
}

// ── Main decoder ──────────────────────────────────────────────────────────
export function decodeSwapTx(tx: TransactionResponse): DecodedSwap | null {
  if (!tx.data || tx.data.length < 10) return null;

  // Must target the Nofeeswap contract
  if (tx.to?.toLowerCase() !== NOFEESWAP_ADDR) return null;

  // Decode outer unlock() call
  let unlockDecoded: ethers.Result;
  try {
  ...
  if (typeof operator !== 'string' ||
      operator.toLowerCase() !== OPERATOR_ADDR) return null;

  // innerData is VM bytecode — parse it
  const rawBytes = ethers.getBytes(innerData as string);
  ...
  // Direction string
  const directionStr =
    zeroForOne === 1n ? `${ADDRESSES.token0Symbol ?? 'TK0'}→${ADDRESSES.token1Symbol ?? 'TK1'}` :
    zeroForOne === 0n ? `${ADDRESSES.token1Symbol ?? 'TK1'}→${ADDRESSES.token0Symbol ?? 'TK0'}` :
    'Auto (2)';
